...
let layers = [{ name: "Layer 1", canvas: document.createElement("canvas") }];
let activeLayer = 0;
...
canvas.addEventListener("wheel", e => {
  e.preventDefault();
  zoom += e.deltaY * -0.001;
  zoom = Math.min(Math.max(.3, zoom), 5);
  applyTransform();
});
...
function floodFill(x,y,color) {
  ...
  let target = data.slice(index,index+4);
...
canvas.addEventListener("click", e => {
  if (tool === "fill") {
    floodFill(e.offsetX, e.offsetY, [0,0,0]);
  }
});
...
function saveDrawing() {
  localStorage.setItem("drawData", canvas.toDataURL());
}

function loadDrawing() {
  let data = localStorage.getItem("drawData");
  if (!data) return;
...
document.getElementById("toolFill").onclick =
  () => tool="fill";

document.getElementById("saveBtn").onclick = saveDrawing;
document.getElementById("loadBtn").onclick = loadDrawing;
